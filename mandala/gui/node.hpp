...
namespace mandala
{
	...
	namespace gui
	{
		struct node_t
		{
			...
			enum class dock_mode_t : uint8_t
			{
                none,
                bottom,
                fill,
			...
			//enum class auto_size_mode_t : uint8_t
			//{
			//	none,
			//	grow_and_shrink,
			//	grow_only
			...
			struct trace_result_t
			{
				...
				std::vector<std::shared_ptr<node_t>> nodes_hit;
			};
			...
			static bool trace(std::shared_ptr<node_t> node, trace_args_t args, trace_result_t& result);
