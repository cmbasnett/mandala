...
void main()
{
		...
		gpu_program_t(vertex_shader_source, fragment_shader_source)
	{
        ...
        region_min_location = gpu.get_uniform_location(get_id(), "region_min");
        region_size_location = gpu.get_uniform_location(get_id(), "region_size");
	}
	...
	void gui_image_gpu_program_t::world_matrix(const mat4_t& world_matrix) const
	{
		gpu.set_uniform(world_matrix_location, world_matrix, false);
	}

	void gui_image_gpu_program_t::view_projection_matrix(const mat4_t& view_projection_matrix) const
	{
		gpu.set_uniform(view_projection_matrix_location, view_projection_matrix, false);
	}

	void gui_image_gpu_program_t::diffuse_texture_index(uint32_t diffuse_texture_index) const
	{
		gpu.set_uniform(diffuse_texture_index_location, diffuse_texture_index);
	}

	void gui_image_gpu_program_t::color(const vec4_t& color) const
	{
		gpu.set_uniform(color_location, color);
	}

    void gui_image_gpu_program_t::region_min(const vec2_t& region_min) const
    {
        gpu.set_uniform(region_min_location, region_min);
    }

    void gui_image_gpu_program_t::region_size(const vec2_t& region_size) const
    {
        gpu.set_uniform(region_size_location, region_size);
    }
}
